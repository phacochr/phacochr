...
phaco_data_update <- function(){

path_data<- paste0(find.package("phacochr"),"/data-raw")
...
# Première fois
if (!file.exists(paste0(path_data,"/BeST/log.csv"))){
  log <- data.frame(update = "0001-01-01 00:00:00 UTC")
  write_csv2(log,paste0(path_data,"/BeST/log.csv"))
  rm(log)
}

log <- read_csv2(paste0(path_data,"/BeST/log.csv"))
log$update <- as.POSIXct(log$update)

if (max(as.Date(log$update)) + days(7) < Sys.Date()) {
  options(timeout=300)

  download.file("https://opendata.bosa.be/download/best/postalstreets-latest.zip",paste0(path_data,"/BeST/postalstreets-latest.zip"))
  download.file("https://opendata.bosa.be/download/best/openaddress-bevlg.zip",paste0(path_data,"/BeST/openaddress-bevlg.zip"))
  download.file("https://opendata.bosa.be/download/best/openaddress-bebru.zip",paste0(path_data,"/BeST/openaddress-bebru.zip"))
  download.file("https://opendata.bosa.be/download/best/openaddress-bewal.zip",paste0(path_data,"/BeST/openaddress-bewal.zip"))

  unzip(paste0(path_data,"/BeST/postalstreets-latest.zip"), exdir= paste0(path_data,"/BeST"))
  file.remove(paste0(path_data,"/BeST/postalstreets-latest.zip"))

  unzip(paste0(path_data,"/BeST/openaddress-bevlg.zip"), exdir= paste0(path_data,"/BeST"))
  file.remove(paste0(path_data,"/BeST/openaddress-bevlg.zip"))

  unzip(paste0(path_data,"/BeST/openaddress-bebru.zip"), exdir= paste0(path_data,"/BeST"))
  file.remove(paste0(path_data,"/BeST/openaddress-bebru.zip"))

  unzip(paste0(path_data,"/BeST/openaddress-bewal.zip"), exdir= paste0(path_data,"/BeST"))
  file.remove(paste0(path_data,"/BeST/openaddress-bewal.zip"))
  ...
  Brussels_postal_street <- read_csv(paste0(path_data,"/BeST/Brussels_postal_street.csv"), col_types = cols(.default = col_character()))
  Wallonia_postal_street <- read_csv(paste0(path_data,"/BeST/Wallonia_postal_street.csv"), col_types = cols(.default = col_character()))
  Flanders_postal_street <- read_csv(paste0(path_data,"/BeST/Flanders_postal_street.csv"), col_types = cols(.default = col_character()))
  ...
  # Bruxelles
  openaddress_bebru <- read_csv(paste0(path_data,"/BeST/openaddress-bebru.csv"), col_types = cols(.default = col_character()))
  openaddress_bebru <- select_id_street(openaddress_bebru)
  openaddress_bebru <- join_ss_adress(openaddress_bebru)
  # Wallonie
  openaddress_bewal <- read_csv(paste0(path_data,"/BeST/openaddress-bewal.csv"), col_types = cols(.default = col_character()))
  openaddress_bewal <- select_id_street(openaddress_bewal)
  openaddress_bewal <- join_ss_adress(openaddress_bewal)
  # Flandres
  openaddress_bevlg <- read_csv(paste0(path_data,"/BeST/openaddress-bevlg.csv"), col_types = cols(.default = col_character()))
  ...
  write_csv2(belgium_street_abv, paste0(path_data,"/PREPROCESSED/belgium_street_abv_PREPROCESSED.csv"))
  ...
  code_postal_INS <- read_excel(paste0(path_data,"/statbel/Statbel_code_postaux/Conversion Postal code_Refnis code_va01012019.xlsx")) %>%
  ...
  write_csv2(table_postal_arrond, paste0(path_data,"/PREPROCESSED/table_postal_arrond.csv"))
  ...
  write_csv2(table_INS_recod_code_postal, paste0(path_data,"/PREPROCESSED/table_INS_recod_code_postal.csv"))
  ...
  for (i in filter_arrondissements) {
    ...
    write_csv2(temp, paste0(paste0(path_data,"/PREPROCESSED/data_arrond_PREPROCESSED_",  i, ".csv")), na = "")
  }
  ...
  BXL_QUARTIERS_sf <- st_read(paste0(path_data,"/urbis/URBIS_ADM_MD/UrbAdm_MONITORING_DISTRICT.shp")) %>%
    st_set_crs(31370)
  ...
  write_csv2(table_secteurs_prov_commune_quartier, paste0(path_data,"/PREPROCESSED/table_secteurs_prov_commune_quartier.csv"), na = "")
  ...
  write_csv2(mat, paste0(path_data,"/PREPROCESSED/table_commune_adjacentes.csv"))
  ...
  #file.remove(c("BeST/Brussels_postal_street.csv",
  #              "BeST/Flanders_postal_street.csv",
   #             "BeST/Wallonia_postal_street.csv"))
  #file.remove("BeST/openaddress-bevlg.csv")
  #file.remove("BeST/openaddress-bebru.csv")
  #file.remove("BeST/openaddress-bewal.csv")
