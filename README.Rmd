...
Un tableau synthétisant les résultats du géocodage est accessible dans l'objet `summary` créé par `phaco_geocode()` :
...
* Vérifier le nombre d'adresses qui ont nécessité une localisation géographique à un autre numéro si celui renseigné n'a pas été trouvé. Examinez plus spécifiquement les adresses qui ont demandé les approximations les plus amples (colonne `approx_num`) ;
* Vérifier le nombre d'adresses qui ont demandé une localisation au numéro médian de la rue au code postal encodé. Examinez plus spécifiquement les adresses qui ont demandé une localisation à la rue (colonne `type_geocoding == mid_street`).
