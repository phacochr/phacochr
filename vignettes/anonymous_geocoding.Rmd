...
`phacochr` a d'abord été pensé pour fonctionner en local : il doit donc être léger, rapide et portable. Cette spécificité permet notamment de géocoder des données sensibles qui ne peuvent pas être envoyées sur un serveur via une API de géocodage. Des options supplémentaires ont de ce fait été intégrées afin d'anonymiser les données qui lui sont passées. Nous en montrons un exemple sur cette page.
